...
public class AddrSpace {
	...
	public int getSwapSpaceLocationFromVP(int virtualPageNumber) {
		return mPageTable[virtualPageNumber].getSwapSpacePageLocation();
	}

	public TranslationEntry getTransEntryFromPageFrame(int phyFrameNumber) {
		for(int i = 0; i < mPageTable.length; ++i) {
			if(mPageTable[i].physicalPage == phyFrameNumber) {
				return mPageTable[i];
			}
		}
		return null;
	...
	public void setTableEntry(int index, TranslationEntry transEntry) {
		mPageTable[index] = transEntry;
	}
	...
	public AddrSpace(OpenFile executable) {
		...
		System.out.println("Virtual Pages count: " + mNumPages + " for: " + JNachos.getCurrentProcess());
		...
		for (int i = 0; i < mNumPages; i++) {
			...
			JNachos.swapFile.writeAt(bytes, Machine.PageSize, JNachos.swapCounter * Machine.PageSize);
			...
			if ((i * Machine.PageSize) < (noffH.code.size + noffH.initData.size)) {
				...
				executable.readAt(bytes, Machine.PageSize, noffH.code.inFileAddr + i * Machine.PageSize);
				System.out.println("Data read for: " + i + " " + bytes.toString());
				
				JNachos.swapFile.writeAt(bytes, Machine.PageSize, JNachos.swapCounter * Machine.PageSize);
				System.out.println("Data written for: " + i + " " + bytes.toString());
			...
			JNachos.swapCounter += 1;
		}
		System.out.println("num of pages written to swap file: " + JNachos.swapCounter);
	}
	...
	public void setProcIdPerPage() {
		// TODO Auto-generated method stub
		for (int i = 0; i < this.mPageTable.length; ++i) {
			this.mPageTable[i].processId = this.processId;
		}
	}
}
