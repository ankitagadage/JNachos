...
public class NachosProcess implements Runnable {
	...
	/**
	 * Resumes this process. NOTE: We piggy back Nachos Processes on top of java
	...
	/**
	 * The run function for the Java-level thread
	 **/
	public void run() {
	...
	public void kill() {
		...
		System.out.println("******* Deleting Process " + mName + "********");
		assert (this != JNachos.getCurrentProcess());
		if (mSpace != null) {
			byte[] clearingBuffer = new byte[Machine.PageSize];
			int killingProcId = this.getmProcessId();
			NachosProcess killingProcess = JNachos.globalProcTable.get(killingProcId);
			System.out.println("Process clearing from SWAPSPACE: " + killingProcess.getName());
			System.out.println("Process clearing from RAM: " + killingProcess.getName());
			// Clearing the exiting process's memory from RAM
			BitMap currProcBmp = killingProcess.getSpace().mFreeMap;
			TranslationEntry[] currProcPageTable = killingProcess.getSpace().getmPageTable();
			for(int i = 0; i < currProcPageTable.length; ++i) {
				try {
					currProcBmp.clear(currProcPageTable[i].physicalPage); // clear the page from RAM
					JNachos.swapFile.writeAt(clearingBuffer, Machine.PageSize, currProcPageTable[i].swapSpacePageLocation * Machine.PageSize); // clear the page from swap space
				}catch(Exception e) {
	...
	public void sleep() {
		...
		// returns when we've been signaled
		JNachos.getCurrentProcess().switchProcess(nextProcess);
	}
	...
	public synchronized void switchProcess(NachosProcess pNextProcess) {
		...
		//System.out.println("\n Switching from oldProc: " + oldProcess + "\n to newProc: " + pNextProcess);
